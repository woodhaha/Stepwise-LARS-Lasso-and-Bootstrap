...
bootIndex <- sample.int(29,  size=29, replace=T)
bootData <- myData[bootIndex,]

numUnique <- length(unique(bootIndex))

# -------------------- ITERATIONS -------------------- #

myX <- as.matrix(bootData[,2:dim(bootData)[2]])
myY <- bootData[,1]
...
plot(t, rep(0,k), type="l", ylim=c(-1.5, 1))
...
rowSums(BETA)[rowSums(BETA)!=0]

library(lars)
larModel <- lars(myX, myY, type="lasso", trace=T)
sysBeta <- t(larModel$beta)[,-1]
colSums(abs(sysBeta))


# plot(seq(1:maxSteps), rep(0,maxSteps), type="l", ylim=c(-5, 5))
# for(i in 1:dm) { lines(seq(1:maxSteps), BETA[i,],, lwd=2, col=sample(colours(),1)) }
# 
# library(ggplot2)
# myBetas <- data.frame(step=rep(1:maxSteps, 150), betas=as.vector(t(BETA)), 
#                       cate=rep(paste0("beta", 1:150), each=maxSteps))
# ggplot(data = myBetas, aes(x=step, y=betas)) + geom_line(aes(colour=cate))